...
const Xsql = require('./xsql.js');
const Xctrl = require('./xctrl.js');
...
const authPassportJWT = (req, res, next) => {
  passport.authenticate('jwt', {session: false}, (err, user, info) => {
    if (err || !user) {
      ...
      if (req.query['_where']) {
        req.query['_where'] = req.query['_where'].replace(/@user_id/, user.id);
      }
      next();
    }
  })(req, res, next);
...
class Xapi {
  ...
  setupRoutes() {
    ...
    // user management
    this.app.post('/auth/register', (req, res, next) => {
      const pool = this.mysql.pool;
    ...
    this.app.post('/auth/login', function(req, res, next) {
    ...
    this.app
      ...
      .get(authPassportJWT, this.asyncMiddleware(this.tables.bind(this)));
    ...
    // filter users
    resources = resources.filter(({resource}) => resource != 'users');
            ...
            if (!this.config.readOnly) {
              this.app
                ...
                .get(
                  authPassportJWT,
                  this.asyncMiddleware(
                    resourceCtrl.bulkRead.bind(resourceCtrl),
                  ),
                );
            } else {
              stat.apis--;
          ...
          case 'relational':
            this.app
              ...
              .get(
                authPassportJWT,
                this.asyncMiddleware(
                  resourceCtrl.nestedList.bind(resourceCtrl),
                ),
              );
            break;
    ...
    if (this.config.dynamic === 1 && !this.config.readOnly) {
      this.app
        .route('/dynamic*')
        .post(authPassportJWT, this.asyncMiddleware(this.runQuery.bind(this)));
